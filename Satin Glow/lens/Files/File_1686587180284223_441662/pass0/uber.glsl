...
#endif
vec4 l9_4=sc_SampleTextureBiasOrLevel(opacityTexDims.xy,opacityTexLayout,l9_3,(varPackedTex.xy*uv2Scale)+uv2Offset,(int(SC_USE_UV_TRANSFORM_opacityTex)!=0),opacityTexTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_opacityTex,SC_SOFTWARE_WRAP_MODE_V_opacityTex),(int(SC_USE_UV_MIN_MAX_opacityTex)!=0),opacityTexUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_opacityTex)!=0),opacityTexBorderColor,0.0,opacityTex);
...
{
if (l9_5<((mod(dot(floor(mod(sc_GetGlFragCoord().xy,vec2(4.0))),vec2(4.0,1.0))*9.0,16.0)+1.0)*0.0588235))
{
discard;
}
}
#endif
vec4 l9_7=ngsPixelShader(l9_6);
...
#if (UseViewSpaceDepthVariant&&((sc_OITDepthGatherPass||sc_OITCompositingPass)||sc_OITDepthBoundsPass))
{
l9_10=varViewSpaceDepth;
}
#else
{
l9_10=sc_ProjectionMatrixArray[sc_GetStereoViewIndex()][3].z/(sc_ProjectionMatrixArray[sc_GetStereoViewIndex()][2].z+((sc_GetGlFragCoord().z*2.0)-1.0));
}
#endif
float l9_11=clamp(l9_10*0.001,0.0,1.0);
...
#if (sc_OITFrontLayerPass)
{
vec4 l9_12=texture2D(sc_OITFrontDepthTexture,getScreenUV());
float l9_13;
#if (sc_SkinBonesCount>0)
{
l9_13=5e-07;
...
#endif
if (abs(sc_GetGlFragCoord().z-l9_12.x)>l9_13)
{
discard;
}
